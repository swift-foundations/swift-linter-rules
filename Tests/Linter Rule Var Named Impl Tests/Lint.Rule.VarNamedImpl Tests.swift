...
@testable import Linter_Rule_Var_Named_Impl
...
extension Lint.Rule.VarNamedImpl.Test {
    static func findings(in source: String, file: String = "test.swift") -> [Lint.Finding] {
        ...
        return Lint.Rule.VarNamedImpl().findings(in: parsed)
    }
}

extension Lint.Rule.VarNamedImpl.Test.Unit {
    @Test
    func `let impl is flagged`() {
        let source = "let impl = make()"
        let findings = Lint.Rule.VarNamedImpl.Test.findings(in: source)
...
extension Lint.Rule.VarNamedImpl.Test.`Edge Case` {
    @Test
    ...
    func `empty file produces no findings`() {
        let findings = Lint.Rule.VarNamedImpl.Test.findings(in: "")
        #expect(findings.isEmpty)
    }
}
