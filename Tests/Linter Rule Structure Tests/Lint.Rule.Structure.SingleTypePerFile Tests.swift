...
extension Lint.Rule.Structure.SingleTypePerFile.Test {
    static func findings(in source: String, file: String = "Sources/X/Test.swift") -> [Diagnostic.Record] {
        ...
        return Lint.Rule.Structure.SingleTypePerFile().findings(in: parsed)
    }
}

extension Lint.Rule.Structure.SingleTypePerFile.Test.Unit {
    @Test
    func `single struct is permitted`() {
        let source = "struct Foo {}"
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: source)
        #expect(findings.isEmpty)
    }

    @Test
    func `single class is permitted`() {
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: "class Foo {}")
        #expect(findings.isEmpty)
    }

    @Test
    func `single enum is permitted`() {
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: "enum Foo {}")
        #expect(findings.isEmpty)
    }

    @Test
    func `single actor is permitted`() {
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: "actor Foo {}")
        #expect(findings.isEmpty)
    }

    @Test
    func `single protocol is permitted`() {
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: "protocol Foo {}")
        #expect(findings.isEmpty)
    }
...
extension Lint.Rule.Structure.SingleTypePerFile.Test.`Edge Case` {
    @Test
    ...
    func `Tests path scope-excluded - multiple types permitted`() {
        ...
        struct B {}
        """
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(
            in: source,
    ...
    func `empty file produces no findings`() {
        let findings = Lint.Rule.Structure.SingleTypePerFile.Test.findings(in: "")
        #expect(findings.isEmpty)
    }
